...
if pkg_installed grub && [ -f /boot/grub/grub.cfg ]; then
    ...
    if [ ! -f /etc/default/grub.t2.bkp ] && [ ! -f /boot/grub/grub.t2.bkp ]; then
        ...
        if [ "${grubtheme}" == "None" ]; then
            ...
            # Use the new install-hyperfluent-theme.sh script
            if [ -f "${scrDir}/install-hyperfluent-theme.sh" ]; then
                cd "${scrDir}" && "${scrDir}/install-hyperfluent-theme.sh"
                if [ $? -eq 0 ]; then
                ...
                echo -e "\033[0;31m[ERROR]\033[0m install-hyperfluent-theme.sh not found"
                grubtheme="None"
            fi
        else
        ...
        if [ "${grubtheme}" != "hyperfluent-arch" ]; then
            ...
            echo -e "\033[0;32m[BOOTLOADER]\033[0m GRUB config already regenerated by hyperfluent-arch installer"
        fi
    else
